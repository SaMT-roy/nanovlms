...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def __init__(self, cfg, activate_lora=False):
        ...
        self.lora_wq = LoraLayer(self.embd_dim, self.embd_dim, name='lora_wq', active=activate_lora)
        self.lora_wv = LoraLayer(self.embd_dim, self.head_dim * self.n_kv_heads, name='lora_wv', active=activate_lora)
    ...
    def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask=None) -> Tuple[torch.Tensor, dict]:
        ...
        q_curr = (self.q_proj(x)+self.lora_wq(x)).view(B, T_curr, self.n_heads, self.head_dim).transpose(1, 2)  # (B, n_heads, T_curr, head_dim)
        k_curr = self.k_proj(x).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2) # (B, n_kv_heads, T_curr, head_dim)
        v_curr = (self.v_proj(x)+self.lora_wv(x)).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2) # (B, n_kv_heads, T_curr, head_dim)
...
class LanguageModel(nn.Module):
    ...
    def generate(self, inputs: torch.Tensor, max_new_tokens: int=20):
        ...
        for i in range(max_new_tokens):
            ...
            prompt_output = self.forward(generated_emb, attention_mask=None)
